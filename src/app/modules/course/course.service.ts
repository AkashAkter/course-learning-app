...
const createCourse = async (teacherId: string, payload: TCourse) => {
  ...
  // Check if the teacherId is valid MongoDB ObjectId
  ...
  if (!teacher) {
    throw new ApiError(httpStatus.NOT_FOUND, `Teacher with ID ${teacherId} not found`);
  }
  
  if (teacher.role !== 'teacher') {
    throw new ApiError(httpStatus.FORBIDDEN, 'Only teachers can create courses');
  }
  
  // Create the course with the teacher's ID
  const courseData = {
    ...payload,
...
const updateCourse = async (
  ...
  // Check if the user is the teacher who owns this course
